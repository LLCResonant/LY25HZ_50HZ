...
void Scib_Q1Command(void)                
{
	...
	bStrLen1 = sbNumToAscii(WorkMode, 0, pDataBuf);    //2ModuleAdd
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

    pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(Calc_Result.f32VGrid_rms, 0, pDataBuf);    //3 ADCorrection.f32VGrid * 1000
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

    pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(Calc_Result.f32IGrid_rms * 100, 0, pDataBuf);    //4  ADCorrection.f32IGrid * 1000
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

    pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(Calc_Result.f32GridFreq * 100, 0, pDataBuf);    //5
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

    pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(Calc_Result.f32VBusP, 0, pDataBuf);    //6
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;
 
    pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(Calc_Result.f32VBusN, 0, pDataBuf);    //7
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

    pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(Calc_Result.f32VInv_rms, 0, pDataBuf);    //8ADCorrection.f32VInv * 1000
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(Calc_Result.f32IInv_rms * 100, 0, pDataBuf);    //9  ADCorrection.f32IInv * 1000
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32; 	

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(Calc_Result.f32VOut_rms, 0, pDataBuf);    //10  ADCorrection.f32VOut * 1000
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;
	
    pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
    bStrLen1 = sbNumToAscii(Calc_Result.f32IOut_rms *100 , 0, pDataBuf);//11 ADCorrection.f32IOut * 1000
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(PowerCon_Reg.f32OutputWatt1 * 100, 0, pDataBuf);//12
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(Calc_Result.f32TempPFC, 0, pDataBuf);//13
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;
	
	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(Calc_Result.f32TempInv, 0, pDataBuf);//14
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;
	
	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii( PowerCon_Reg.f32OutputWatt, 0, pDataBuf);//15
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(ShortCheck_Reg.Restart_time_interval, 0, pDataBuf);//16
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(fabs(OutPLLConReg.f32Theta - VOutPLLReg.f32Theta) * 1000, 0, pDataBuf);//17ShortCheck_Reg.Restart_times
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;
...
/////////////////////////////
    ...
    pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(g_SysWarningMessage.Word.byte0,0, pDataBuf);//26 s_u16Cnt_DcFan1_Hi_Level
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

    pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(g_SysWarningMessage.Word.byte1, 0, pDataBuf);//27 s_u16Cnt_DcFan1_Low_Level
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

    pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(g_StateCheck.Word.byte0, 0, pDataBuf);  //28 s_u16Cnt_DcFan2_Hi_Level
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;
	

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(g_StateCheck.Word.byte1, 0, pDataBuf);//29 ADCorrection.f32TempPFC * 100
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(g_StateCheck.Word.byte2, 0, pDataBuf);//30 ADCorrection.f32TempPFC * 100
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(g_StateCheck.Word.byte3, 0, pDataBuf);//31 ADCorrection.f32TempPFC * 100
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(g_StateCheck.Word.byte4, 0, pDataBuf);//32 ADCorrection.f32TempPFC * 100
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(g_StateCheck.Word.byte5, 0, pDataBuf);//33 s_u16Cnt_DcFan2_Low_Levelg_StateCheck.Word.byte4 g_Mod_Statusg_StateCheck.Word.byte5
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(g_StateCheck.Word.byte6, 0, pDataBuf);//34 s_u16Cnt_FanCtrl_Hi_Level g_StateCheck.Word.byte5
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(ADCorrection.f32VOut *1000, 0, pDataBuf);//35 s_u16Cnt_FanCtrl_Hi_Level g_StateCheck.Word.byte5
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(ADCorrection.f32VOut*1000, 0, pDataBuf);//36 s_u16Cnt_FanCtrl_Hi_Level g_StateCheck.Word.byte5
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;
	
	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(ADCorrection.f32TempPFC*1000, 0, pDataBuf);//37  g_StateCheck.Word.byte6 s_u16Cnt_FanCtrl_Low_LevelHWADFault   //  swGetOutputCurrRMS_w() s_f32LoadPower_Transformer
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32; 

    pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(ADCorrection.f32TempPFC*1000, 0, pDataBuf);//38// 2017.3.7 GX  g_MPPT_State.Word.byte0
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

    pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(ADCorrection.f32TempPFC*1000, 0, pDataBuf);//39// 2017.3.7 GX  g_MPPT_State.Word.byte0
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;

	pDataBuf = &u8Scib_UserDataBuf0[bStrLen];
	bStrLen1 = sbNumToAscii(ADCorrection.f32IGrid*1000, 0, pDataBuf);//40  //2017.3.3 GX g_MPPT_State.Word.byte1
	bStrLen += bStrLen1;
	u8Scib_UserDataBuf0[bStrLen++] = 32;
...
int16 swGetFault1(void)
{
  ...
  return (GetRealValue.f32IGrid * 100);//  2 CurrConReg.f32PfcDuty_ffBusCon_Reg.f32BusVoltDiff_Out * 10 CurrConReg.f32PfcDuty_ff VGrid_Clarke.f32Beta; IGrid_Clarke.f32Beta   //SinValueB
}
...
int16 swGetFault4(void)
{
	return (GetRealValue.f32IInv * 100);//5  GridPLLConReg.f32real_Sin_Theta Calc_Result.f32VBusN * 1GetRealValue.f32VGrid*10CurrConReg.f32PfcDuty GridPLLConReg.Sin_Theta * 400 GX 2017.2.20 GetRealValue.f32IGrid //IGrid_Clarke.f32Beta * 40
}

int16 swGetFault5(void)
{
  return ( GetRealValue.f32VInv); //6  CurrConReg.f32PfcDuty_Con * 1 GetRealValue.f32VGrid*10//BusCon_Reg.f32IGridAmp_Ref * IGrid_Clarke.f32Beta * 40
}


int16 swGetFaultUnrecover1(void)
{
	return(GetRealValue.f32VOut); //7 GetRealValue.f32IGrid_R * 100
}
int16 swGetFaultUnrecover2(void)
{
	return(InvCurrConReg.f32InvDuty); //8   GetRealValue.f32IGrid_S * 100
}
int16 swGetRLineVolt(void)
{
	return(GetRealValue.f32TempPFC);//9
}
int16 swGetSLineVolt(void)
{
...
int16 swGetTLineVolt(void)
{
	return( Calc_Result.f32VGrid_ave*10);//11
}
int16 swGetRCurr(void)
{
	return(Calc_Result.f32VGrid_ave*10);//12
}
