...
Uint8 i = 0;	
	...
	u16TxBuf[i++] = Calc_Result.f32VInv_rms;

	if (Calc_Result.f32IOut_rms * 10 < 5)
		u16TxBuf[i++] = 0;
	else
		u16TxBuf[i++] = (Uint16)(Calc_Result.f32IOut_rms * 10 + 0.5);
	if (g_Sys_Current_State == NormalState )
		u16TxBuf[i++] = Calc_Result.f32VOutLdFreq * 10;
	else
		u16TxBuf[i++] = 0;

	u16TxBuf[i++] = 1;//50Hz  0：25Hz
	u16TxBuf[i++] = Calc_Result.f32TempInv * 10;
	u16TxBuf[i++] = 0;
	u16TxBuf[i++] = 0;
	u16TxBuf[i++] = Calc_Result.f32VOutFreq * 10;
	u16TxBuf[i++] = 90;
	u16TxBuf[i++] = 0;
...
void SetVaule_ControlRefresh()
{
 ...
 {
	 u16TxBuf[0] = SafetyReg.f32Inv_VoltRms_Ref_LCD;
	 u16TxBuf[1]	= SafetyReg.f32Inv_VoltRms_Ref_LCD;
 }
 else
 {
	 u16TxBuf[0] = SafetyReg.f32Inv_VoltRms_Ref;
	 u16TxBuf[1]	= SafetyReg.f32Inv_VoltRms_Ref;
 }
	...
	if (g_Mod_Status == Master)
	{
		if(address == 0)
		{
			if( u16RxBuf[0] > 211 && u16RxBuf[0] < 229 )
				SafetyReg.f32Inv_VoltRms_Ref_LCD = u16RxBuf[0];
		}
		else if(address == 1)
		{
			if( u16RxBuf[1] > 211 && u16RxBuf[1] < 229)
				SafetyReg.f32Inv_VoltRms_Ref_LCD = u16RxBuf[1];
		}
		else
			;
...
void ComRcvDeal()
{
	...
	{	
		...
		{  
		    ...
		    {
		    	if (ComRcvData[3] >= 1 && ComRcvData[3] <= 8 && u8_Num == 0) //2017.12.23 GX
		    	{
