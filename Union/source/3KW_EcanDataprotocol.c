...
void Broadcast(void)
{
	...
	if (g_Mod_Status == Master)
	{
		EcanP2A_Tx.P2AMail_id.bit.bus = 0x0;
 		if ( SafetyReg.u16Short_Restart_times == 0 && 1 == g_StateCheck.bit.Inv_SoftStart && NormalState == g_Sys_Current_State \
 				&& g_StateCheck.bit.Bypwork == 0)
 		{
 			Output_Revise();
...
{
	Uint16 u16_sourcetype = 0;
	u16_sourcetype = P2A_RX.P2AMail_id.bit.source_type;

	if ( ModuleAdd != 0X00 && u16_sourcetype != 0x0C && \
			g_Sys_Current_State != FaultState  && g_Sys_Current_State != PermanentState )
	{
		...
		{
			if ( (SafetyReg.u16Short_Restart_times == 0) && 1 == g_StateCheck.bit.Inv_SoftStart && (NormalState == g_Sys_Current_State)  &&  \
					P2A_RX.P2AMail_id.bit.bus == 0  && g_StateCheck.bit.Bypwork == 0)//
			{
	...
	if ( u16_sourcetype == 0x0C)
	{
		Calc_Result.f32flag ++;
		if (P2A_RX.P2AMail_id.bit_Byp.bus == 0x1)
		{
			g_StateCheck.bit.Bypwork = 1;
			g_StateCheck.bit.Bypforce = 0;
		}
		else if (P2A_RX.P2AMail_id.bit_Byp.bus == 0x2)
		{
			g_StateCheck.bit.Bypwork = 1;
			g_SysFaultMessage.bit.Bypoffline = 1;
		}
		else
		{
			g_StateCheck.bit.Bypforce = 0;
