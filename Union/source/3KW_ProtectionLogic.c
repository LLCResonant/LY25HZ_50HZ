...
void  InvHVoltCheck(void)
{
	...
	{
		if (Calc_Result.f32VInvH_rms < SafetyReg.f32VInvH_LowLimit && (1 == g_StateCheck.bit.Inv_SoftStart) \
				&& g_StateCheck.bit.Bypwork == 0) //202V
		{
...
void  InvLVoltCheck(void)
{
	...
	{
		if (Calc_Result.f32VInvL_rms < SafetyReg.f32VInvL_LowLimit && (1 == g_StateCheck.bit.Inv_SoftStart)\
				 && g_StateCheck.bit.Bypwork == 0)//101V
		{
	...
	if  ( 1 == g_StateCheck.bit.Inv_SoftStart && Calc_Result.f32VOutH_rms>= 190 )
	{
		...
		if (s_u8temp1 >= 20)  //20 * 40ms = 800ms
		{
			if(fabs(OutPLLConReg.f32Theta - VOutHPLLConReg.f32Theta) >=  s_f32phase_diff && \
					g_ParaLogic_State.bit.SyncPhase_Flag == 1 && g_Sys_Current_State != PermanentState && \
					g_Sys_Current_State != FaultState && g_StateCheck.bit.Bypwork == 0)
			{
				s_u8temp2 ++;
				if(s_u8temp2 >= 10)
				{
