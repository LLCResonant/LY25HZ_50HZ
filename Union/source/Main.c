...
Uint8 	u8ecan_cnt = 1;
Uint8 	u8hosttemp = 0;
enum SYS_STRUCTURE	 g_Sys_Structure_State = Single;
enum SYS_BYPASS	g_Sys_Bypass_State = noBypass;
...
{		
    ...
    if ((g_Sys_Current_State == FaultState || g_Sys_Current_State == PermanentState) && s_temp == 0)
    {
    	Times_WriteToEEPROM();
    ...
    else if (g_Sys_Current_State != FaultState && g_Sys_Current_State != PermanentState)
    {
    	s_temp = 0;
    }
    else
