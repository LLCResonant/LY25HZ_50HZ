...
void InvL_VoltControl(void)
{
    ...
    EPwm5Regs.CMPA.half.CMPA = (Uint16)(-InvLVoltConReg.f32InvDuty);  //2018.12.23 GX
...
{ 
	  ...
	  */
	if (g_StateCheck.bit.Bypwork == 1)
	{
		OutPLLConReg.f32Theta +=  GridPLLConReg.f32Theta_Step;
		//OutPLLConReg.f32Theta = GridPLLConReg.f32Theta;
	}
	else
	{
	 ...
	 */
	if(1 == g_ParaLogic_State.bit.SelfPhaseOut_EN && g_StateCheck.bit.Bypwork == 0 &&\
			g_Sys_Current_State != PermanentState && g_Sys_Current_State != FaultState)
	{
...
void InvVoltSlowup(void)
{
	...
	if(g_StateCheck.bit.Bypwork == 1)
	{
		SafetyReg.f32InvH_VoltRms_Ref = Calc_Result.f32VGrid_rms + 5;  //2018.12.27 GX
		u8_count = 0;
	}
	if(g_StateCheck.bit.Bypwork == 0 && u8_count <= 2 \
			&& g_Sys_Bypass_State == Bypass)//2018.12.24 GX
	{
		if (u8_count == 2)
		{
...
void InverterStage_Init(void)
{
	...
	if(g_StateCheck.bit.Bypwork == 1)
	{
		SafetyReg.f32InvH_VoltRms_Ref = Calc_Result.f32VGrid_rms + 5; //2018.12.27
		OutPLLConReg.f32Theta = GridPLLConReg.f32Theta;
	}
	else
	{
