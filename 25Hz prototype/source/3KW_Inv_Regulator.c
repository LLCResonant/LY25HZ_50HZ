...
void InvH_VoltControl(void)
{
	 ...
	 */
	/*if(Calc_Result.f32IInvH_rms_previous <= 0.9f)
	{
		kp = 0.8f;
		kr = 800* 0.00005f;
	}
	else if(Calc_Result.f32IInvH_rms_previous > 0.9f && Calc_Result.f32IInvH_rms_previous <= 1.3f)
	{
		kp=kp;
		kr=kr;
	}
	else if(Calc_Result.f32IInvH_rms_previous > 1.3f && Calc_Result.f32IInvH_rms_previous <= 3.85f)
	{
		kp = 0.6f;
		kr = 400* 0.00005f;
	}
	else if(Calc_Result.f32IInvH_rms_previous > 3.85f && Calc_Result.f32IInvH_rms_previous <= 4.15f)
	{
		kp=kp;
		kr=kr;
	}
	else if(Calc_Result.f32IInvH_rms_previous > 4.15f)
	{
		kp = 0.8f;
		kr = 800* 0.00005f;
	}*/

	InvHVoltConReg.f32VoltInst_Ref = (InvHVoltConReg.f32VoltRms_Ref) \
			* 1.414f * OutPLLConReg.Sin_Theta * InvHVoltConReg.f32VoltGain;
	...
	InvHVoltConReg.f32VoltInst_ErrNew = InvHVoltConReg.f32VoltInst_Ref - InvHVoltConReg.f32VoltInst_Fdb;
  	 ...
  	 //used for hold-up time, which is abandoned now
  	 //InvHCurrConReg.f32InvDuty = Calc_Result.Coff_Dforward2 * InvHVoltConReg.f32VoltInst_ErrOut;
...
void InvL_VoltControl(void)
{
	 ...
	 *	Change INVH(220V) PR parameter according to output current amplitude.
	 */
	/*if(Calc_Result.f32IInvL_rms_previous <= 0.9f)
	{
		kp = 2.0f;
		kr = 1200* 0.00005f;
	}
	else if(Calc_Result.f32IInvL_rms_previous > 0.9f && Calc_Result.f32IInvL_rms_previous <= 1.1f)
	{
	    kp=kp;
	    kr=kr;
	}
	else if(Calc_Result.f32IInvL_rms_previous > 1.1f && Calc_Result.f32IInvL_rms_previous <= 4.8f)
	{
		kp = 2.0f;
		kr = 350* 0.00005f;
	}
	else if(Calc_Result.f32IInvL_rms_previous > 4.8f && Calc_Result.f32IInvL_rms_previous <= 5.1f)
	{
		kp=kp;
		kr=kr;
	}
	else if(Calc_Result.f32IInvL_rms_previous > 5.1f)
	{
		kp = 1.5f;
		kr = 1200* 0.00005f;
	}*/

	InvLVoltConReg.f32VoltInst_Ref = (InvLVoltConReg.f32VoltRms_Ref) * \
			1.414f * OutPLLConReg.Cos_Theta * InvLVoltConReg.f32VoltGain;
	...
	//used for hold-up time, which is abandoned now
	//InvLCurrConReg.f32InvDuty = Calc_Result.Coff_Dforward2 * InvLVoltConReg.f32VoltInst_ErrOut;
	...
	if((g_StateCheck.bit.Inv_SoftStart == 1) && (NormalState==g_Sys_Current_State))
	{
		if(ShortCheck_Reg.Restart_times >=1)
		{
			...
			else
			{
				...
				SafetyReg.f32InvH_VoltRms_Ref = InvH_VoltRef;
			    SafetyReg.f32InvL_VoltRms_Ref = InvL_VoltRef;
				//InvHVoltConReg.f32VoltRms_Ref = SafetyReg.f32InvH_VoltRms_Ref;
				//InvLVoltConReg.f32VoltRms_Ref = SafetyReg.f32InvL_VoltRms_Ref;
			}
		}
